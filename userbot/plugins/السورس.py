...
♛ : ping  {ping}  𓇡.
♛ : version 7.6  𓇡.
♛ : Source تيلثون الزنوج : @vivvvvvd 𓇡."""
...
async def amireallyalive(event):
    ...
    IQTHON_ALIVE_TEXT = "❬ تـليثون الزنوج - Telethon-alznog ، 🕸  ❭ :"
...
async def _(event):
    ...
    OUTPUT = (f"♛︙  وحـدة المعلومات الخاصه بتنصيبك مع جميع الفارات  لتنصيب سورس الزنوج @vivvvvvd :**\n\n{o}")
...
async def _(event):
    ...
    await asyncio.sleep(1)
    await edit_or_reply(event , "حسنا جاري تحديث تليثون الزنوج انتضر من 5 الى 10 دقائق")
    try:
...
♛  اوامر الملصقات ↢ ( .م16 )
———————×———————
شرح محد بي حيل : ( @vivvvvvd ) .
قناه السورس : ( @telethonalznog ) .
...
None
———————×——————— 
قناه المتغيرات او الفارات : @nothing
شرح الأوامر : ( @nothinf ) .
قناه السورس : ( @telethonalznog ) .
جميع الاوامر تكون بدايتها نقطة .
""")
...
@check_owner
async def inlineiqthon(iqthon):
    text = "**♛︙ قـائمـه الاوامـر :**\n**♛︙ قنـاه السـورس :** @telethonalznog\n**♛︙ شـرح اوامـر السـورس : @none**\n**♛︙ شـرح فـارات السـورس : @telethonalznog** "
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlineiqthon(iqthon):
        ...
        if query.startswith("اوامر الصيغ(?: |$)(.*)") and iqthon.query.user_id == bot.uid:
            ...
            result = builder.article(title="telethonalznog", text=help2, buttons=buttons, link_preview=False)
            await iqthon.answer([result] if result else None)
